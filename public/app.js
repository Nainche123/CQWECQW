...
function settingsHTML() {
  ...
  return `<div class="section first"><div class="section-title"><div><span class="eyebrow">SETTINGS</span><h2>설정</h2><p>현재 계정의 판매 정보와 연결된 Discord 자판기만 관리합니다. **NEXIVO 제작자 오너 ID와 Bot Token은 구매자에게 요구하지 않습니다.**</p></div><button class="btn primary" id="save-settings">${icon('check',13)} 변경사항 저장</button></div>${ownerConfig}<div class="settings-grid"><div class="card"><div class="card-head"><div><h3>스토어 기본 설정</h3><p>고객 안내에 사용할 정보를 입력하세요. 이 정보는 다른 판매자와 공유되지 않습니다.</p></div></div><div class="form-grid"><label>상점 이름<div class="input bare"><input id="set-store-name" value="${esc(s.storeName || '')}" placeholder="예: NEXIVO HUB STORE"></div></label><label>운영자 표시 이름<div class="input bare"><input id="set-admin-name" value="${esc(s.adminName || '')}" placeholder="예: 홍길동 / NEXIVO 운영팀"></div></label><label class="full-row">입금 안내<textarea id="set-bank" placeholder="예: 국민은행 123-456-789012 / 홍길동">${esc(s.bankInfo || '')}</textarea><small class="form-help">충전 신청 시 이 계좌가 현재 라이선스의 Discord 서버에 표시됩니다.</small></label><label class="full-row">서버 초대 링크<div class="input bare"><input id="set-server-invite" value="${esc(s.serverInviteUrl || '')}" placeholder="https://discord.gg/..."></div></label><label>Discord Guild ID<div class="input bare"><input id="set-guild" value="${esc(s.guildId || '')}" placeholder="예: 123456789012345678"></div></label><label>주문 카테고리 ID<div class="input bare"><input id="set-order-cat" value="${esc(s.orderCategoryId || '')}" placeholder="선택"></div></label><label>로그 채널 ID<div class="input bare"><input id="set-log" value="${esc(s.logChannelId || '')}" placeholder="선택"></div></label><label>구매자 역할 ID<div class="input bare"><input id="set-role" value="${esc(s.buyerRoleId || '')}" placeholder="선택"></div></label><label>재고 채널 ID<div class="input bare"><input id="set-stock-channel" value="${esc(s.stockChannelId || '')}" placeholder="재고 안내를 표시할 채널 ID"></div></label><label class="full-row">주문 카테고리 이름<div class="input bare"><input id="set-order-cat-name" value="${esc(s.orderCategoryName || '')}" placeholder="선택"></div></label><label class="full-row bot-switch-row"><span><b>웹 ↔ Discord 재고 자동 동기화</b><small>웹에서 상품/재고를 변경하면 연결된 서버의 봇 자판기에 자동 반영됩니다.</small></span><input id="set-stock-sync" type="checkbox" ${s.stockSyncEnabled !== false ? 'checked' : ''}></label></div></div><div class="card"><div class="card-head"><div><h3>내 봇 연결 상태</h3><p>이 계정의 라이선스와 Discord 서버 연결 정보를 확인합니다.</p></div></div><div class="safety-list"><div><span class="status-inline good">●</span><div><b>라이선스에 연결된 Discord ID</b><small>${lic.discordUserId ? esc(lic.discordUserId) : (isOwner() ? '오너 계정' : 'Discord에서 `/라이센스` 인증을 완료하면 자동 연결됩니다.')}</small></div></div><div><span class="status-inline ${state.bot?.connected ? 'good' : 'warn'}">●</span><div><b>공용 봇 상태</b><small>${state.bot?.connected ? `${esc(state.bot?.bot?.botUsername || 'NEXIVO HUB Bot')} · 연결됨` : '연결 대기 / 봇 워커가 서버를 확인하는 중입니다.'}</small></div></div><div><span class="status-inline good">●</span><div><b>토큰 보안</b><small>브라우저와 일반 구매자에게 Bot Token을 전달하지 않습니다.</small></div></div>${botCfg.inviteUrl ? `<div><span class="status-inline good">●</span><div><b>봇 초대</b><small><a class="inline-link" href="${esc(botCfg.inviteUrl)}" target="_blank" rel="noreferrer">공용 봇 초대 링크 열기 ↗</a></small></div></div>` : ''}</div></div></div></div>`;
}
...
function renderIssuedLicenseList(){
  ...
  return list.map(l=>`<div class="license-row issued-row ${l.owner ? 'owner-license-row' : ''}"><div class="lr-head"><div><b>${esc(l.owner ? 'OWNER PRO PREMIUM' : (l.planLabel || planName(l.plan)))}</b><span class="status-pill ${statusClass(l.status)}">${esc(statusLabel(l.status))}</span>${l.owner ? '<span class="status-pill ok">SYSTEM</span>' : ''}</div><small>${l.remainingDays===null?'무기한':number(l.remainingDays)+'일 남음'}</small></div><code>${esc(l.key)}</code><div class="issued-meta"><span>${l.owner ? '오너 기본 라이선스' : (l.discordUserId ? `Discord ${esc(l.discordUserId)}` : 'Discord 미연결 · 최초 인증 대기')}</span><span>${l.orderId ? `주문 ${esc(l.orderId)}` : (l.owner ? '오너 계정' : '주문 미지정')}</span><span>발급 ${date(l.createdAt)}</span></div><div class="license-actions">${l.owner ? '<small class="license-system-note">OWNER PRO 라이선스는 보안 계층에서만 변경됩니다.</small>' : `<button class="btn tiny" data-issue-op="copy" data-id="${esc(l.id)}">${icon('copy',12)} 복사</button>${l.status==='ACTIVE'?`<button class="btn tiny" data-issue-op="suspend" data-id="${esc(l.id)}">정지</button>`:''}${l.status==='SUSPENDED'||l.status==='UNUSED'?`<button class="btn tiny" data-issue-op="resume" data-id="${esc(l.id)}">재개</button>`:''}<button class="btn tiny" data-issue-op="extend" data-id="${esc(l.id)}">연장</button><button class="btn tiny" data-issue-op="rotate-secret" data-id="${esc(l.id)}">시크릿</button>${l.status!=='REVOKED'?`<button class="btn tiny danger" data-issue-op="revoke" data-id="${esc(l.id)}">취소</button>`:''}`}</div></div>`).join('');
}
...
async function saveSettings(){
  const payload={storeName:document.getElementById('set-store-name')?.value.trim() || '', adminName:document.getElementById('set-admin-name')?.value.trim() || '', bankInfo:document.getElementById('set-bank').value, serverInviteUrl:document.getElementById('set-server-invite')?.value.trim() || '', guildId:document.getElementById('set-guild').value.trim(), orderCategoryId:document.getElementById('set-order-cat').value.trim(), logChannelId:document.getElementById('set-log').value.trim(), buyerRoleId:document.getElementById('set-role').value.trim(), orderCategoryName:document.getElementById('set-order-cat-name').value.trim(), stockChannelId:document.getElementById('set-stock-channel')?.value.trim() || '', stockSyncEnabled:document.getElementById('set-stock-sync')?.checked !== false};
...
function shopOrderCard(o) {
  const done = String(o.status) === '완료';
  const claimReady = done && o.claimStatus === 'READY';
  return `<div class="shop-order-card"><div><span class="eyebrow">ORDER</span><b>${esc(o.productName)}</b><small>${esc(o.id)} · ${datetime(o.createdAt)}</small></div><div class="shop-order-right"><strong>${money(o.amount)}</strong><span class="status-pill ${statusClass(o.status)}">${esc(o.status)}</span></div>${done?`<div class="shop-delivery"><div><span>라이선스 지급</span><strong>${claimReady?'1회용 발급 링크 준비됨':(o.claimStatus==='USED'?'발급 링크 사용 완료':'지급 완료')}</strong><small>라이선스 키는 링크를 통해 1회만 확인할 수 있습니다.</small></div><div class="delivery-actions">${claimReady?`<button class="btn tiny primary" data-claim-order="${esc(o.id)}">🔑 라이선스 발급 링크</button>`:''}${o.review?`<span class="review-done">⭐ 후기 작성 완료</span>`:`<button class="btn tiny" data-review-order="${esc(o.id)}">⭐ 후기 작성</button>`}</div></div>`:o.status==='입금대기'?`<div class="shop-order-note">지갑 결제 완료 후 라이선스 키가 자동 발급됩니다.</div>`:''}</div>`;
}
function shopHTML() {
...
async function openShopPurchase(productId) {
  ...
  b.querySelector('#sc-buy').onclick=async()=>{try{if(!consent?.checked)throw new Error('서비스 약관과 개인정보 처리방침을 확인해주세요.');if(!enough)throw new Error('잔액이 부족합니다. Discord 자판기에서 충전해주세요.');const d=await api('/api/shop/orders',{method:'POST',body:JSON.stringify({productId})});close();await loadShop();render();showShopOrderReceipt({...d.order,claimUrl:d.claimUrl,claimStatus:d.claimOneTime?'READY':d.order?.claimStatus});}catch(e){toast(e.message||'구매에 실패했습니다.','error');}};
}

function showShopOrderReceipt(order){
  const hasReview=Boolean(order?.review);
  const b=document.createElement('div');b.className='modal-backdrop';b.innerHTML=`<div class="modal order-receipt"><div class="receipt-icon">${icon('check',28)}</div><span class="eyebrow">PURCHASE COMPLETE</span><h2>구매가 완료되었습니다.</h2><p>결제가 완료되었고 라이선스가 준비되었습니다.</p><div class="receipt-row"><span>주문번호</span><code>${esc(order.id)}</code></div><div class="receipt-row"><span>상품</span><b>${esc(order.productName)}</b></div><div class="receipt-row"><span>금액</span><b>${money(order.amount)}</b></div>${order.claimUrl?`<div class="receipt-payment"><span>라이선스 발급</span><div class="license-receipt-key"><a class="btn tiny primary" href="${esc(order.claimUrl)}" target="_blank" rel="noopener">🔑 1회용 발급 페이지 열기</a></div><small class="field-hint">구매 계정으로 로그인한 상태에서 열어야 하며, 발급 페이지는 한 번만 사용할 수 있습니다.</small></div>`:''}<div class="receipt-review-cta">${hasReview?`<div class="review-done">⭐ 후기를 이미 작성했습니다.</div>`:`<button class="btn primary full" id="receipt-review">⭐ 후기 작성하기</button>`}</div><button class="btn full" id="receipt-close">확인</button></div>`;document.body.appendChild(b);b.querySelector('#receipt-close').onclick=()=>b.remove();b.querySelector('#receipt-review')?.addEventListener('click',()=>{b.remove();openShopReviewModal(order.id);});
}
...
function bind(){
  ...
  if(state.area==='shop'){
    ...
    document.querySelectorAll('[data-claim-order]').forEach(b=>b.onclick=async()=>{try{const d=await api('/api/shop/orders/'+encodeURIComponent(b.dataset.claimOrder)+'/claim-link',{method:'POST'});if(d.claimUrl){window.open(d.claimUrl,'_blank','noopener');toast('1회용 라이선스 발급 페이지를 열었습니다.');}else throw new Error('발급 링크를 만들 수 없습니다.');}catch(e){toast(e.message||'라이선스 발급 링크를 만들지 못했습니다.','error');}});
